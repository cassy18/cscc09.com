...
(function () {
  ...
  const state = {
    chirps: [],
    // which chirp is currently being edited. One source of truth instead of
    // cross-component pub/sub — every component reads from this on render.
    activeChirpId: null,
  };
  ...
  function ChirpComponent(chirp) {
    ...
    // render the component's visual state from state.activeChirpId.
    // this is the only place that decides whether the edit form is shown.
    if (state.activeChirpId === chirp.id) {
    ...
    // clicking on edit: just update state and re-render. No cross-component messaging.
    newChirp.querySelector(".edit").addEventListener("click", function () {
      state.activeChirpId = chirp.id;
      updateChirpList();
    });
